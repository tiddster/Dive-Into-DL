...
def batch_print(epoch, batch, loss, train_accuracy, test_accuracy=None, test_loss=None):
    if test_accuracy and test_loss:
        print(
            f"[epoch: {epoch},  batch: {batch} ] ==== train_loss: {loss}, test_loss:{test_loss} ==== train_accuracy: %.4f, test_accuracy: %.4f" % (
            train_accuracy, test_accuracy))
    else:
        print(f"[epoch: {epoch},  batch: {batch} ] ==== loss: {loss} ==== accuracy: %.4f" % train_accuracy)


def epoch_print(epoch, loss_sum, train_accuracy, start, end, test_accuracy=None, test_loss=None):
    if test_accuracy and test_loss:
        print(
            f"[epoch:{epoch}] === train_loss_sum:{loss_sum}, test_loss:{test_loss} === train_accuracy: %.4f, test_accuracy: %.4f === time:{end - start}" % (
            train_accuracy, test_accuracy))
    else:
        print(
            f"[epoch:{epoch}] === train_loss_sum:{loss_sum} === train_accuracy: % .4f === time:{end - start}" % train_accuracy)
